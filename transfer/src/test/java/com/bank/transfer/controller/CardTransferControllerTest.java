...
class CardTransferControllerTest extends ParentTest {
    ...
    void readNegativeTest() throws Exception {
        ...
        when(service.read(anyLong())).thenThrow(
                new EntityNotFoundException("CardTransfer с указанным id не найден"));
    ...
    void updateNegativeTest() throws Exception {
        ...
        when(service.update(ArgumentMatchers.any(), anyLong())).thenThrow(
                new EntityNotFoundException("CardTransfer для обновления с указанным id не найден"));

        mock.perform(put(String.format("/card/update?id=%s", ID))
                        .content(mapper.writeValueAsString(transferDto))
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpectAll(status().isNotFound(), content()
                        .string("CardTransfer для обновления с указанным id не найден"));
    }
}
